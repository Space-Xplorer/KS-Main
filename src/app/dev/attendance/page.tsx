 ...
 *
 * This is a test tool, not the real walk-in desk or scanner UI. It exercises
 * the ACTUAL layered code end to end: real signed tokens, real Dexie outbox,
 * real SyncEngine, real Supabase writes through the apply_checkin() trigger.
 * Nothing here is mocked. What is missing is auth, the provisioning runbook,
 * and any visual polish — those are the real UI's job, later.
 *
 * Off by default everywhere, including this page itself: both this component
 * and every route it calls stay locked until ALLOW_DEV_HARNESS=true is set
 * (see src/lib/dev-guard.ts for why that is a flag and not a NODE_ENV check).
 */
...
export default function DevAttendancePage() {
  // Mirrors src/lib/dev-guard.ts's reasoning: gated on an explicit flag, not
  // NODE_ENV, since Vercel sets NODE_ENV=production on every deployment it
  // builds (including previews), which would make a NODE_ENV check permanent
  // dead code rather than an actual switch.
  if (process.env.NEXT_PUBLIC_ALLOW_DEV_HARNESS !== "true") {
